...
feat(ui): implement noir investigation desk layout
refactor(ui): split investigation desk into feature panels
feat(events): subscribe to session SSE updates
feat(api): map dialogue response contracts to view models
chore(build): ignore generated frontend artifacts
```
